...
from modules.viz_recommender import VisualizationRecommender
import os
import asyncio
...
if 'layout_suggestions' not in st.session_state:
    st.session_state.layout_suggestions = None
...
with st.sidebar:
    ...
    tool_choice = st.radio(
        "Choose Tool:",
        ["Data Quality Analysis", "DAX Generator", "Data Warehouse Designer", "Visualization Recommender"]
    )
...
if uploaded_file is not None:
    try:
        ...
        if tool_choice == "Data Quality Analysis":
        ...
        elif tool_choice == "Visualization Recommender":
            st.header("📊 Visualization Recommender")
            
            # Initialize Visualization Recommender
            viz_recommender = VisualizationRecommender(api_key=API_KEY)
            data_analysis = viz_recommender.analyze_data_for_viz(df)
            
            # Get Visualization Recommendations
            if st.button("Get Visualization Recommendations"):
                with st.spinner("Getting recommendations..."):
                    st.session_state.viz_recommendations = asyncio.run(
                        viz_recommender.get_visualization_recommendations(data_analysis)
                    )
            ...
            # Get Dashboard Layout Suggestions
            if st.button("Get Dashboard Layout Suggestions"):
                with st.spinner("Getting layout suggestions..."):
                    if st.session_state.viz_recommendations:
                        st.session_state.layout_suggestions = asyncio.run(
                            viz_recommender.suggest_dashboard_layout(st.session_state.viz_recommendations)
                        )
                    else:
                        st.warning("Please generate visualization recommendations first.")
            
            if st.session_state.layout_suggestions:
                st.markdown("### Suggested Dashboard Layout")
                st.markdown(st.session_state.layout_suggestions)
    ...
    - 📊 **Visualization Recommender**: Visualization and dashboard layout suggestions
